...
class SphinxQuerySet(object):
    ...
    def _get_sphinx_results(self):
        ...
        def _handle_filters(filter_list, exclude=False):
            for name, values in six.iteritems(filter_list):
        ...
        if isinstance(self._index, six.text_type):
            ...
            if results['matches']:
                ...
                if results['matches'][0]['attrs'].get(pks[0].column):
                    ...
                    for r in results['matches']:
                        r['id'] = ', '.join([six.text_type(r['attrs'][p.column]) for p in pks])
                    ...
                    for r in results['matches']:
                        r['id'] = six.text_type(r['id'])
                    queryset = queryset.filter(pk__in=[r['id'] for r in results['matches']])
                queryset = dict([(', '.join([six.text_type(getattr(o, p.attname)) for p in pks]), o) for o in queryset])
                    ...
                    if results['matches'][0]['attrs'].get(pks[0].column):
                        for r in results['matches']:
                            if r['attrs']['content_type'] == ct:
                                val = ', '.join([six.text_type(r['attrs'][p.column]) for p in pks])
