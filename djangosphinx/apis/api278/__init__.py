...
class SphinxClient(object):
	def __init__(self):
		...
		self._socket = None
		self._offset = 0  # how much records to seek from result-set startdefault is 0)
	...
	def SetLimits(self, offset, limit, maxmatches=0, cutoff=0):
		...
		"""
		assert(type(offset) in six.integer_types and 0 <= offset < 16777216)
		assert(type(limit) in six.integer_types and 0 < limit < 16777216)
		assert(maxmatches >= 0)
		self._offset = offset
		self._limit = limit
		if maxmatches > 0:
